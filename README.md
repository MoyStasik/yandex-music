...
+ static-music.yandex.ru - данный домен будет использоваться для передачи музыки, подкастов, обложек
...
В Москву и Санкт-Петербург можно поставить сервера, чтобы раздавали медиафайлы.
...
Таблица 6 - Распределение запросов "Авторизация/регистрация", "Поиск исполнителя, трека, альбома", "Поиск по жанру" и других  по датацентрам
<table>
    <tr>
        <th>Датацентры</th>
        <th>Средний рпс 890</th>
    </tr>
    <tr>
        <td>Москва</td>
        <td>222,5</td>
...
Таблица 7 - Распределение запросов "воспроизведение музыки", "воспроизведение подкастов" по датацентрам
<table>
    <tr>
        <th>Датацентры</th>
        <th>Средний рпс</th>
...
Тк больше всего пользователей яндекс музыки находится в Москве и Санкт-Петербурге, то ДЦ расположенные в этих городах будут испытывать наибольшую нагрузку. Поэтому будет разумно объединить эти ДЦ под одним IP и использовать BGP Anycast.
...
+ До Nginx трафик будет балансироваться с помощью LVS via Direct Routing;
...
+ Ставим 2n балансировщиков - для каждого резервный.
