...
<table>
    ...
    <tr>
        ...
        song_id - id песен, которые находятся в плейлисте<br>
    ...
    <tr>
        <td>Podcasts</td>
        <td>id - уникальный id плейлиста<br>
        author_id - id автора, которому принадлежит подкаст<br>
        thematic_id - id тематики подкаста<br>
        podcast_name - название подкаста<br>
        description - описание подкаста<br>
        podcast_file_path - путь до файла с подкастом, хранящегося в s3<br>    
        created_at - дата создания подкаста<br>
        updated_at - дата обновления подкаста</td>
...
<table>
    ...
    <tr>
        <td>History</td>
        <td>Раннее мы выяснили, что около 6048 треков хранятся в истории прослушиваний, тогда будет 6048 разных song_id, рассчитаем размер хранилища: 24млн * 6048 * (8 + 8 + 8 + 8 + 8)байт = 5,29Тб</td>
    </tr>
    <tr>
        <td>Thematics</td>
...
<img width="927" alt="image" src="https://github.com/user-attachments/assets/5df21b88-0c37-4c26-9b31-1fe813e76667" />
...
<table>
    ...
    <tr>
        <td>media_files</td>
        <td>Для хранения медиа файлов(треков, подкастов) был выбран Tarantool, тк он позволяет быстро читать данные, в данном случае скорость важна, хочу отметить, что, также, можно было использовать Redis, но Tarantool более надежен и чуть быстрее в записи и чтении</td>
    </tr>
    <tr>
        <td>Остальные таблицы</td>
        <td>Для таблиц users, playlists была выбрана MongoDB, тк она достаточно хорошо работает с высокими нагрузками, хорошо масштабируется</td>
    </tr>
</table>

Также треки, подкасты, предлагаю хранить с помощью Tarantool, тк треки и подкасты в сумме занимают не так много места, а именно 4,03ГБ + 0,003ГБ = 4,033ГБ. Для данного случая преимущества Tarantool над Redis будут такими же, как в представленном раннее обосновании такого выбора.
...
Рекомендации будут формироваться на основе плейлистов пользователя и история прослушиваний. 

Использовать плейлисты, как метрику будет более приоритетным, тк пользователь сам добавил этот трек - значит он понравился. Рассчет будет таков, по трекам в плейлисте будет формироваться список жанров, которые слушает пользователь, те 10 треков были альтернативный рок, 20 хип-хоп и так далее, все жанры считаем и складываем, получаем итоговое число - 100 процентов по всем жанрам, дальше для каждого отдельного жанра будет считаться метрика, например итоговое число получилось 150, а хип-хоп 15 треков в плейлисте, тогда 15/150 = 10% доля на хип хоп, так считаем для каждого жанра и получаем процентаж для каждого жанра. Когда пользователь будет просматривать плейлист дня/мою волну, то будем делать выборку, когда будем загружать треки пользователю то в них будет, как в примере 10% треков на хип-хоп и так по каждому жанру, треки будут относиться к исполнителям, которых слушает пользователь + похожие исполнители - будет собираться статистика по пользователям, у которых похожие вкусы - совпадают исполнители в одинаковых жанрах и исполнители, которых они слушают будут рекомендоваться нашему пользователю. 

Что касаемо истории, будет считать треки, которые прослушивались хотя бы 30 секунд, не особо много смысла считать те, которые пользователь слушал меньше, тк скорее всего трек не понравился. Треки, которые пользователь слушал в течение года, будут оцениваться от 4 до 6, если трек был прослушен до минуты, то вес +4, если от минуты до двух, то вес +5, если более или полностью дослушал, то +6 к весу, метрики будут считаться также, как в предыдущем методе, если пользователь слушал трек более года назад, то веса будут распределяться от 1 до 3 в зависимости от количества минут, вес +1 до 1 мин, +2 до 2 мин, +3 более 2 минут или полностью.

При добавлении пользователем нового трека в плейлист/историю статистика будет пересчитываться, для конкретного жанра увеличивать количество и пересчитывать процентаж для каждого жанра.

**Добавление прослушиваний в историю**

Тк запись в историю происходит очень часто, каждый трек у каждого пользователя будет записываться в БД, то лучше всего будет суммарно собирать до 10к новых записей истории в буфере(kafka) и потом только пушить в ClickHouse.
